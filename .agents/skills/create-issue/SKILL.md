...
1. 你讀的若是 `~/.agents/skills/create-issue/SKILL.md`（全域版），先看 repo 根（`git rev-parse --show-toplevel`）：有 `.agents/skills/create-issue/SKILL.md`，或有 `.claude/skills/create-issue/SKILL.md` 且它不是只轉交到 `.agents/skills/create-issue/SKILL.md` 的薄 stub → **改讀 repo 那份照做，本檔以下全部不適用**。專案版通常客製過（label、tracker、流程），全域版只在專案沒有時補位。
